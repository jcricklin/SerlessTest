...
def sauvtoGCS(destination_blob_name, mon_objet):
    ...
    print(f"Fichier {temp_file_name} uploadé vers {destination_blob_name} dans le bucket {bucket_name}.")
...
def add_sell_price(df_final_reduced) : 
    df_final_reduced['PRIX_VENTE'] = np.where(
        ...
        0,
        np.round((df_final_reduced['PARAM_MARGE'] * (1 + df_final_reduced['TVA']) + df_final_reduced['PRIX_ACHAT']) / df_final_reduced['QUANTITE'], 2)
    )
...
def good_date(df_final) :
    df_final["DATE_IMPORT"] = df_final["DATE_IMPORT"].apply(lambda x: pd.to_datetime(x, format="%Y-%m-%d") if len(x) == 10 else pd.to_datetime(x, format="%Y-%m-%d %H:%M:%S"))
...
def add_ds_unique_id(df_reset) :
    ...
    df_reset = df_reset[[x for x in df_reset.columns if x not in ['ID_PRODUIT', 'DATE_IMPORT']]]
    return df_reset
...
def control_timeId_Matching(X_df) :
  ...
  index = pd.MultiIndex.from_product([unique_ids, forecast_horizon], names=['unique_id', 'ds'])
...
def create_features(df, target_col, id_col='unique_id'):
    ...
    df['rolling_mean_lag1_window_size7'] = df.groupby(id_col)['lag7'].transform(lambda x: x.rolling(window=7).mean())
    df['rolling_max_lag1_window_size7'] = df.groupby(id_col)['lag7'].transform(lambda x: x.rolling(window=7).max())
    df['rolling_min_lag1_window_size7'] = df.groupby(id_col)['lag7'].transform(lambda x: x.rolling(window=7).min())

    df['rolling_mean_lag2_window_size15'] = df.groupby(id_col)['lag15'].transform(lambda x: x.rolling(window=15).mean())
    df['rolling_max_lag2_window_size15'] = df.groupby(id_col)['lag15'].transform(lambda x: x.rolling(window=15).max())
    df['rolling_min_lag2_window_size15'] = df.groupby(id_col)['lag15'].transform(lambda x: x.rolling(window=15).min())

    df['rolling_mean_lag3_window_size30'] = df.groupby(id_col)['lag30'].transform(lambda x: x.rolling(window=30).mean())
    df['rolling_max_lag3_window_size30'] = df.groupby(id_col)['lag30'].transform(lambda x: x.rolling(window=30).max())
    df['rolling_min_lag3_window_size30'] = df.groupby(id_col)['lag30'].transform(lambda x: x.rolling(window=30).min())
