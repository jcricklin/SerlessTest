...
def PrepareDataForDarts(
    ...
    COVARIATES = [
        x
        for x in df_final_test_singleProduct.columns
        if x not in [TIME_COL, TARGET] + STATIC_COLS
    ]
    SCALER = Scaler()
    TRANSFORMER = StaticCovariatesTransformer()
    PIPELINE = Pipeline([SCALER, TRANSFORMER])

    total_darts = TimeSeries.from_group_dataframe(
        df=df_final_reduced_single_product,
    ...
    train_darts = TimeSeries.from_group_dataframe(
        df=df_final_train_singleProduct,
        group_cols=STATIC_COLS,
        time_col=TIME_COL,
        value_cols=TARGET,
        freq=FREQ,
    ...
    test_darts = TimeSeries.from_group_dataframe(
        df=df_final_test_singleProduct,
        group_cols=STATIC_COLS,
        time_col=TIME_COL,
        value_cols=TARGET,
        freq=FREQ,
    ...
    for ts in tqdm(total_darts):
        ...
        covariate = TimeSeries.from_dataframe(
            df_final_reduced_single_product[
                df_final_reduced_single_product["unique_id"] == unique_id
            ][[x for x in df_final_reduced_single_product.columns if x != "QUANTITE"]],
            time_col=TIME_COL,
        ...
        other_cov = TimeSeries.from_dataframe(
            df_final_reduced_single_product[
                (df_final_reduced_single_product["unique_id"] == unique_id)
            ],
            time_col=TIME_COL,
...
def TrainSaveDartsTFT(train_transformed, covariates_transformed, nb_epoch, num_id):
    ...
    tft_model = tft_model.fit(
        train_transformed, future_covariates=covariates_transformed, verbose=True
    )
...
def download_blobs(bucket_name, model_name, prefix, temp_dir):
    ...
    for blob in blobs:
        ...
        file_path = os.path.join(
            temp_dir, prefix, model_name, blob.name.replace(prefix + "/", "")
        )
...
def mean_absolute_scaled_error_tft_darts(
    y_true,
    y_pred,
    y_train,
    ids,
    ...
    for id_val in ids:
        ...
        pred = [
            pred
            for pred in y_pred
            if int(pred.static_covariates["unique_id"]) == id_val
        ]
        ...
        if isinstance(x, int):
            ...
            if x == 7:
                new_index_entries[f"rolling_mean_lag1_window_size{x}"] = (
                    calcule_rolling_mean_lag(x, df_for_output)
                )
                new_index_entries[f"rolling_max_lag1_window_size{x}"] = (
                    calcule_rolling_max_lag(x, df_for_output)
                )
                new_index_entries[f"rolling_min_lag1_window_size{x}"] = (
                    calcule_rolling_min_lag(x, df_for_output)
                )
...
def tft_Explanation(
    num_id, modelTFTload, train_transformed, train_darts, create_covariates
):
    total_dico_impTFT_cluster = []
    for id_produit in tqdm([num_id], desc="Processing products"):
        explainer = TFTExplainer(
            modelTFTload,
            background_series=train_transformed[
                [
                    i
                    for i in range(len(train_darts))
                    if int(train_darts[i].static_covariates["unique_id"]) == id_produit
                ][0]
            ],
            background_future_covariates=create_covariates[
                [
                    i
                    for i in range(len(train_darts))
                    if int(train_darts[i].static_covariates["unique_id"]) == id_produit
        ...
        somme_attention = sum(
            [
                sum(explainability_result.get_attention()[y].values()[0])
                for y in range(30)
            ]
        )
        ...
        dict_transit = [
            {"variable": element, "importance": data[element][0]} for element in data
        ]
...
def function_result_final(
    ...
    tft_model, temp_dir, gcs_bucket_name, gcs_model_dir, gcs_checkpoint_dir = (
        TrainSaveDartsTFT(train_transformed, covariates_transformed, nb_epoch, num_id)
    )

    test = tft_model.predict(
        n=90,
    ...
    errorMetric = mean_absolute_scaled_error_tft_darts(
        df_final_test_singleProduct[
            df_final_test_singleProduct["ds"]
            >= df_final_test_singleProduct["ds"].max() - pd.Timedelta(days=89)
        ],
        prediction,
        df_final_train_singleProduct,
        [num_id],
        for_index="tft_produit",
    )

    final_feature_importance_df_tft_cluster = tft_Explanation(
        num_id, tft_model, train_transformed, train_darts, create_covariates
    )

    tft_produit_result = final_feature_importance_df_tft_cluster.merge(
        errorMetric, on="unique_id", how="left"
    )
...
def save_csv_to_gcs(destination_blob_name, dataframe):
    ...
    print(
        f"File {temp_file_name} uploaded to {destination_blob_name} in bucket {bucket_name}."
    )
...
def main_training_TFTModelProduitII(
    nb_epoch,
    num_id,
    df_final_test_singleProduct,
    df_final_train_singleProduct,
    ...
    tft_produit_result = function_result_final(
        nb_epoch,
        num_id,
        df_final_test_singleProduct,
        df_final_train_singleProduct,
    ...
    study_name_dateAujoudhui = (
        "ErrorProduit_detail" + f"{num_id}" + f"_{DATE_UPDATE_optuna}"
    )
