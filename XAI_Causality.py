...
def create_feature_importance_table(df_final_train_shap, shap_values_df, list_produit_test, feature_comparition):
    all_feature_importance = []

    for unique_id in [x for x in df_final_train_shap['unique_id'].unique() if x in list_produit_test]:
...
def prepare_data_for_tigramite(df):
    ...
    pp_frame = pp.DataFrame(data=df.values,
...
def time_averaged_tigramite_results(results, var_names, threshold=0.1):
    ...
    mi_pairs = [(var_names[i], var_names[j], upper_tri[i, j])
...
def prepar_for_output(df_for_output) :
    ...
    for x in df_for_output.index:
        if isinstance(x, int):
            ...
            if x == 7:
                new_index_entries[f'rolling_mean_lag1_window_size{x}'] = calcule_rolling_mean_lag(x)
                new_index_entries[f'rolling_max_lag1_window_size{x}'] = calcule_rolling_max_lag(x)
                new_index_entries[f'rolling_min_lag1_window_size{x}'] = calcule_rolling_min_lag(x)

            if x == 15:
                new_index_entries[f'rolling_mean_lag2_window_size{x}'] = calcule_rolling_mean_lag(x)
                new_index_entries[f'rolling_max_lag2_window_size{x}'] = calcule_rolling_max_lag(x)
                new_index_entries[f'rolling_min_lag2_window_size{x}'] = calcule_rolling_min_lag(x)

            if x == 30:
                new_index_entries[f'rolling_mean_lag3_window_size{x}'] = calcule_rolling_mean_lag(x)
                new_index_entries[f'rolling_max_lag3_window_size{x}'] = calcule_rolling_max_lag(x)
                new_index_entries[f'rolling_min_lag3_window_size{x}'] = calcule_rolling_min_lag(x)
...
def analyze_quantity_effects(results, var_names, alpha_level=0.05, min_effect_size=0.1, max_lag=5):
    val_matrix = results['val_matrix']
...
def causal_impact_on_quantity(causal_df, var_names, max_lag=5):
    ...
    impact_df = impact_df.sort_values('abs_value', ascending=False).drop('abs_value', axis=1)
...
def analyse_with_MIC(df_final_reduced, unique_id)  :
    tau_max = 30
    pc_alpha = 0.01
    unselect = UnselectedColunnes(df_final_reduced[df_final_reduced['unique_id']==unique_id])
    quantite_uniqueProduit = df_final_reduced[df_final_reduced['unique_id'] == unique_id][[x for x in df_final_reduced.columns if x not in unselect + ['PARAM_MARGE','TVA','ds', 'unique_id']]]
    ...
    for lag in range(results['val_matrix'].shape[2]):
        mi_pairs, mi_df = interpret_3d_tigramite_results(results, [x for x in quantite_uniqueProduit.columns if x not in ['unique_id', 'ds']], lag=lag)
    ...
    df_for_output = pd.DataFrame(importance_MIC+importance_lag_total, columns=['variable','importance'])
